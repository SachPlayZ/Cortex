...
export async function POST(request: Request, { params }: { params: Promise<{ invoiceId: string }> }): Promise<Response> {
  ...
  try {
    const prepared = await new CasperLifecycleService().prepareList(invoiceId, body.public_key_hex, body.account_hash);
    return Response.json(prepared);
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Unable to prepare list" }, { status: 400 });
  }
}

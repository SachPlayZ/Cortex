...
export async function POST(request: Request, { params }: { params: Promise<{ invoiceId: string }> }): Promise<Response> {
  ...
  try {
    const invoice = await new CasperLifecycleService().confirmClaim(invoiceId, body.intent_id, body.deploy_hash);
    return Response.json({ invoice });
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "Unable to confirm claim" }, { status: 400 });
  }
}
